...
**This is not a new abstraction. It is the description of what every device already does.**

Every I/O device in a computer works by the same mechanism: the device writes data into a memory region, and the CPU reads from it. A NIC receives an Ethernet frame and DMAs it into a ring buffer in RAM. A GPU completes a computation and writes the result into a framebuffer. An NVMe SSD controller writes a sector into a host-side bounce buffer. A SPI sensor shifts bytes into a FIFO register. A keyboard controller sets a status bit in an I/O port. In every case: one producer writes to a declared memory location, one or more consumers read from it.

The channel declaration is not an abstraction *on top of* this physical reality. It is the exact description of it, named and typed. The NIC's ring buffer is a `channel EthernetFrame` with `backing = dma`, `tier = task`, and `size` equal to the ring depth. The GPU output buffer is a `channel TensorResult`. The SPI FIFO is a `channel SpiReading`. These are not metaphors — they are the same physical structure with the same ownership model, expressed in a form the compiler can reason about.

The conventional OS hides this behind file descriptors, socket buffers, `recv()`, interrupt handlers, DMA coordination code, and driver stacks — because without declared timing, the question "when does this memory location contain valid data?" has no compile-time answer. The entire stack of abstractions exists to answer that question at runtime, one layer at a time, with compensation for every layer that could not answer it. With declared timing, the question has a compile-time answer: the data is valid when the producer's window closes, and the consumer's window opens no earlier than the declared coherency gap after that. The channel is the memory location. The `@Timeslice` is the answer to when. The driver stack, the interrupt handler, and the socket buffer are the compensation machinery that becomes unnecessary once the question is answered before the first instruction executes.
...
**Every data structure is a channel whose access pattern encodes the key-to-index mapping.** This sounds abstract until you notice that most programmers already know the simplest case and simply never named it: **an array is a map whose key is the index**. `array[i]` is `map.get(i)` where the hash function is the identity. There is no semantic difference — only a syntactic one that hides what is actually happening. Once you see it, the rest of the zoo collapses: a ring buffer is a map whose key is a counter modulo size; a hash table is a map whose key is passed through a mixing function; a sorted lookup table is a map whose read key triggers a binary search. A FIFO, a LIFO, a sliding window, a priority index — all are maps. All are channels. The only variable is what arithmetic the access pattern applies to the key before producing a slot number.
