...
**If a circuit cannot run this tick, it runs next tick.** A circuit whose input channel is empty has no data to process. The runtime skips its window. The core moves to the next dispatch table entry. There is no retry mechanism, no blocking call, no wait queue, no backpressure protocol to implement. At the next tick where the circuit's period falls due, the runtime checks the channel again. If data has arrived in the meantime, the circuit executes. If not, it is skipped again. The circuit is absent from execution until the wire has something on it.
