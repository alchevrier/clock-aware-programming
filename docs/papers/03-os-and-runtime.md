...
This closes the loop between architecture and implementation that software engineering has pursued since the first CASE tools in the 1980s. The reason it was never achieved is that the diagram lived at a level of abstraction above the concurrency model, and the concurrency model was where all the correctness conditions lived. Remove the concurrency model — replace it with declared channels and proved windows — and the gap between the diagram and the program closes to zero.
