...
**When the data genuinely does not fit — you use `cold` and pay once.** If a dataset is too large for DRAM, it belongs in the `cold` tier (NVMe, off-core storage). The difference from `mmap` is the difference between a declared cost and a lottery. With `mmap`, the page fault happens at an arbitrary access site, at an unpredictable time, in the middle of a hot path, interrupting the pipeline with a kernel entry that may take millions of cycles. With `cold`, the compiler inserts a declared gap in the dispatch table — a window boundary before the circuit that needs the data — during which the NVMe driver circuit issues the DMA, waits the declared transfer time, and signals completion via a channel write. The consuming circuit opens its window only after the data is resident. The cost is paid exactly once, at the declared window boundary, bounded by the NVMe transfer latency in `system.cap`, visible in the manifest, and never paid again until the data is demoted back to `cold`. There is no page fault. There is no kernel entry. There is no pipeline flush. The compiler put the cost where it belongs — before the computation that needs the data — and the runtime executes the pre-computation that makes the computation fast.
