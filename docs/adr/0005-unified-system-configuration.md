...
- `system.cap` supports a `topology` field that selects the deployment model: `partitioned` (default — OS cores + app cores) or `app-only` (bare metal, no OS partition). In `app-only` mode the compiler verifies that no declared circuit references a kernel service channel, and the build produces a bootloader-loadable binary with no OS substrate. The same `system.cap` schema, the same proc-macro, and the same signed manifest apply across both topologies — the topology is a declaration, not a different programming model.
